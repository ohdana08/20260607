...
import { dedupePrograms } from "./dedupePrograms";
...
// programs 테이블에 중앙 6소스와 허용된 지역 원천(경기기업비서)을 미리 적재해두고,
...
export async function fetchOpenPrograms(): Promise<{ programs: Program[]; usingSample: boolean }> {
  ...
  const openPrograms = dedupePrograms(programs.filter((program) => isStillOpen(program.applyEnd)));

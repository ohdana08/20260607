...
create schema if not exists ddakfit_monitor_private;
revoke all on schema ddakfit_monitor_private from public, anon, authenticated;

create table if not exists ddakfit_monitor_private.backup_monitor_dispatches (
  request_id bigint primary key,
  requested_at timestamptz not null default clock_timestamp(),
  completed_at timestamptz,
  status_code integer,
  outcome text not null default 'pending'
    constraint backup_monitor_dispatches_outcome_check
    check (outcome in ('pending', 'healthy', 'backup_detected', 'auth_failed', 'monitor_unavailable', 'response_missing')),
  constraint backup_monitor_status_code check (status_code is null or status_code between 100 and 599)
);

revoke all on table ddakfit_monitor_private.backup_monitor_dispatches
  from public, anon, authenticated, service_role, ddakfit_backup_preview;

create or replace function ddakfit_monitor_private.dispatch_backup_monitor()
returns bigint
language plpgsql
security definer
...
declare
  ...
  signed_at text;
  signature text;
  dispatched_id bigint;
begin
  select decrypted_secret into strict monitor_secret
  from vault.decrypted_secrets
  where name = 'ddakfit_backup_monitor_secret';

  if length(monitor_secret) < 32 then
    raise exception using errcode = '22023', message = 'backup_monitor_secret_invalid';
  end if;

  signed_at := floor(extract(epoch from clock_timestamp()))::bigint::text;
  signature := encode(extensions.hmac(
    convert_to(signed_at, 'UTF8'),
    convert_to(monitor_secret, 'UTF8'),
    'sha256'
  ), 'hex');

  select net.http_get(
    url := 'https://20260607.vercel.app/api/cron/monitor-backup',
    headers := jsonb_build_object(
      'Accept', 'application/json',
      'X-Ops-Monitor-Timestamp', signed_at,
      'X-Ops-Monitor-Signature', signature
    ),
    timeout_milliseconds := 12000
  ) into dispatched_id;

  insert into ddakfit_monitor_private.backup_monitor_dispatches(request_id)
  values (dispatched_id);
...
revoke all on function ddakfit_monitor_private.dispatch_backup_monitor()
  from public, anon, authenticated, service_role, ddakfit_backup_preview;
